...
export type ResonatorModule = {
  ...
  feedbackTone: BiquadFilterNode; feedbackGain: GainNode; wet: GainNode; dry: GainNode; output: GainNode
...
export function createResonatorModule(audioContext: AudioContext, settings: ResonatorSettings): ResonatorModule {
  const module = {
    ...
    shaper: audioContext.createWaveShaper(), feedbackTone: audioContext.createBiquadFilter(), feedbackGain: audioContext.createGain(),
  ...
  module.shaper.connect(module.feedbackTone).connect(module.feedbackGain).connect(module.filter)
...
export function destroyResonatorModule(module: ResonatorModule): void {
  ...
  module.feedbackTone.disconnect(); module.feedbackGain.disconnect(); module.wet.disconnect(); module.dry.disconnect(); module.output.disconnect()
}
